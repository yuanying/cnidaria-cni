...
func policy(name string, mode v1alpha1.Mode, spec v1alpha1.NodeNetworkPolicySpec) v1alpha1.NodeNetworkPolicy {
	spec.Mode = mode
	return v1alpha1.NodeNetworkPolicy{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: spec}
}

func ipBlock(cidr string, except ...string) v1alpha1.NodeNetworkPolicyPeer {
	return v1alpha1.NodeNetworkPolicyPeer{IPBlock: &networkingv1.IPBlock{CIDR: cidr, Except: except}}
}

func port(proto corev1.Protocol, p int32) v1alpha1.NodeNetworkPolicyPort {
	return v1alpha1.NodeNetworkPolicyPort{Protocol: proto, Port: ptr(p)}
}
...
var goldenCases = []struct {
...
}{
	{
		...
		name: "not-selected",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("storage", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				...
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{
					{Ports: []v1alpha1.NodeNetworkPolicyPort{port(corev1.ProtocolTCP, 3260)}},
				},
			}),
		},
	},
	{
		...
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("metrics", v1alpha1.ModePermissive, v1alpha1.NodeNetworkPolicySpec{
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
					From:  []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.0/24")},
					Ports: []v1alpha1.NodeNetworkPolicyPort{port(corev1.ProtocolTCP, 9100)},
				}},
			}),
		},
	},
	{
		...
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("control-plane", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				NodeSelector: metav1.LabelSelector{
					MatchLabels: map[string]string{"node-role.kubernetes.io/control-plane": ""},
				},
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
					From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("198.51.100.0/24"), ipBlock("2001:db8:2::/64")},
	...
	},
	{
		...
		name: "mixed-modes",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("ssh-only", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
					Ports: []v1alpha1.NodeNetworkPolicyPort{port(corev1.ProtocolTCP, 22)},
				}},
			}),
			policy("metrics", v1alpha1.ModePermissive, v1alpha1.NodeNetworkPolicySpec{
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
	...
	},
	{
		...
		name: "ipblock-except",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("lan", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
					...
					Ports: []v1alpha1.NodeNetworkPolicyPort{port(corev1.ProtocolTCP, 10250)},
				}},
			}),
		},
	},
	{
		...
		name: "port-range",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("services", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
					Ports: []v1alpha1.NodeNetworkPolicyPort{
	...
	},
	{
		...
		name: "egress-only",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("upstream", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
				PolicyTypes: []v1alpha1.PolicyType{v1alpha1.PolicyTypeEgress},
				Egress: []v1alpha1.NodeNetworkPolicyEgressRule{
					{
						To:    []v1alpha1.NodeNetworkPolicyPeer{ipBlock("203.0.113.0/24")},
						Ports: []v1alpha1.NodeNetworkPolicyPort{port(corev1.ProtocolTCP, 443)},
					},
					{
						To: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("2001:db8:3::/64")},
					},
				},
	...
	},
	{
		...
		name: "deny-all",
		policies: []v1alpha1.NodeNetworkPolicy{
			policy("lockdown", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
...
// The table a node ends up with is the whole of what a NodeNetworkPolicy does, so its shape
...
func TestAddReportsTheModeEachPolicyTookEffectIn(t *testing.T) {
	byName := map[string][]v1alpha1.NodeNetworkPolicy{}
	for _, tc := range goldenCases {
...
func TestAPolicyEnforcingOnlyOneDirectionReadsAsPermissive(t *testing.T) {
	both := policy("both", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
		PolicyTypes: []v1alpha1.PolicyType{v1alpha1.PolicyTypeIngress, v1alpha1.PolicyTypeEgress},
	})
	watching := policy("watching", v1alpha1.ModePermissive, v1alpha1.NodeNetworkPolicySpec{
		PolicyTypes: []v1alpha1.PolicyType{v1alpha1.PolicyTypeEgress},
	})
	got, err := Add(base(t), node, []v1alpha1.NodeNetworkPolicy{both, watching})
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
...
func TestEgressIsClosedOnlyWhenItHasRulesOrIsListed(t *testing.T) {
	ingressOnly := policy("ingress-only", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
		Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{}},
	})
	rs := base(t)
	if _, err := Add(rs, node, []v1alpha1.NodeNetworkPolicy{ingressOnly}); err != nil {
		t.Fatalf("Add: %v", err)
	}
	...
	withEgress := policy("with-egress", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
		Egress: []v1alpha1.NodeNetworkPolicyEgressRule{{}},
	})
	rs = base(t)
	if _, err := Add(rs, node, []v1alpha1.NodeNetworkPolicy{withEgress}); err != nil {
		t.Fatalf("Add: %v", err)
	}
	if text := rs.String(); !strings.Contains(text, ChainOutputDispatch) {
...
func TestAddSelectsWithMatchExpressions(t *testing.T) {
	workers := policy("workers", v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{
		NodeSelector: metav1.LabelSelector{MatchExpressions: []metav1.LabelSelectorRequirement{{
			Key:      "node-role.kubernetes.io/control-plane",
			Operator: metav1.LabelSelectorOpDoesNotExist,
		}}},
	})
	got, err := Add(base(t), node, []v1alpha1.NodeNetworkPolicy{workers})
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
	if len(got) != 0 {
...
func TestAddRejectsBadInput(t *testing.T) {
	cases := map[string]v1alpha1.NodeNetworkPolicySpec{
		"peer without an ipBlock": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{From: []v1alpha1.NodeNetworkPolicyPeer{{}}}},
		},
		"cidr that is not a prefix": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.1")}}},
		},
		"cidr with bits below its prefix length": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.1/24")}}},
		},
		"except with bits below its prefix length": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.0/24", "192.0.2.129/25")},
			}},
		},
		"except of another family": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.0/24", "2001:db8::/64")},
			}},
		},
		"except that is not a prefix": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				From: []v1alpha1.NodeNetworkPolicyPeer{ipBlock("192.0.2.0/24", "nonsense")},
			}},
		},
		"endPort without a port": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				Ports: []v1alpha1.NodeNetworkPolicyPort{{Protocol: corev1.ProtocolTCP, EndPort: ptr[int32](80)}},
			}},
		},
		...
		"port out of range": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				Ports: []v1alpha1.NodeNetworkPolicyPort{{Protocol: corev1.ProtocolTCP, Port: ptr[int32](70000)}},
			}},
		},
		"protocol nft has no keyword for": {
			Ingress: []v1alpha1.NodeNetworkPolicyIngressRule{{
				Ports: []v1alpha1.NodeNetworkPolicyPort{{Protocol: corev1.Protocol("HTTP")}},
			}},
		},
	...
	for name, spec := range cases {
		t.Run(name, func(t *testing.T) {
			rs := base(t)
			before := rs.String()
			if _, err := Add(rs, node, []v1alpha1.NodeNetworkPolicy{policy("bad", v1alpha1.ModeEnforce, spec)}); err == nil {
...
func TestALongNameIsCarriedRatherThanRefused(t *testing.T) {
	...
	rs := base(t)
	if _, err := Add(rs, node, []v1alpha1.NodeNetworkPolicy{
		policy(long, v1alpha1.ModeEnforce, v1alpha1.NodeNetworkPolicySpec{}),
	}); err != nil {
